...
const AlarmCard: React.FC<AlarmCardProps> = ({ time, onClick }) => {
    // 시간 문자열(HH:mm)에서 AM/PM 판별
    const parsedHour = time ? parseInt(time.split(':')[0], 10) : NaN;
    const amPm = !isNaN(parsedHour) ? (parsedHour >= 12 ? 'PM' : 'AM') : null;
    ...
    return (
        ...
        >
            ...
            <div className="flex items-baseline gap-2">
                ...
                {amPm && <span className="text-xl text-slate-500">{amPm}</span>}
            </div>
